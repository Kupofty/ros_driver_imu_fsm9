...
int main(int argc, char **argv) {
  ...
  message.dataModeControlV2Request.formatSelect = FORMAT_SELECT; // Format
  ...
  while (ros::ok()) {
    ...
    if (message.messageType == FREESPACE_MESSAGE_MOTIONENGINEOUTPUT) {
      ...
      if (have_accel && have_angVel && have_angPos) {
        ...
        msg.orientation.z = -angPos.z/norm;
        msg.orientation.w = angPos.w/norm;
#else
        msg.orientation.x = angPos.y/norm;
        msg.orientation.y = angPos.x/norm;
        msg.orientation.z = -angPos.z/norm;
        msg.orientation.w = angPos.w/norm;
#endif
